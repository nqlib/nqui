...
/**
 * Public list props must extend Radix List (not the core TabsList wrap).
 * Wrapping our own forwardRef drops className / variant / children in the
 * published declaration file (0.7.8–0.7.9).
 */
...
const EnhancedTabsList = React.forwardRef<HTMLDivElement, EnhancedTabsListProps>(
  ({ className, orientation, variant = "default", ...props }, ref) => {
    ...
    return (
      <CoreTabsList ref={ref} variant={variant} className={listClassName} {...props} />
    )
  }
)
...
const EnhancedTabsContent = React.forwardRef<HTMLDivElement, EnhancedTabsContentProps>(
  ({ className, ...props }, ref) => {
    return (
      <CoreTabsContent
        ref={ref}
        className={cn("transition-opacity duration-[var(--duration-standard)] ease-[var(--ease-in-out)]", className)}
        {...props}
      />
    )
  }
